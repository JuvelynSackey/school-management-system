...
// assistantIntents.config.js has zero allowedRoles entries for 'student' —
// a student's question always ends up at aiQuery.controller.js's hard
// refusal path (a normal 200 with a graceful "not available" answer, never
// an error), so showing the entry point to them is safe even though nothing
// in it can currently answer their question.
const ASK_JESMANAGE_ROLES = ['admin', 'teacher', 'parent', 'student'];
...
export default function AppShell() {
  ...
  return (
    <div className="app-shell">
        ...
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            {ASK_JESMANAGE_ROLES.includes(user?.role) && (
              <button type="button" className="btn-secondary" onClick={() => setAskOpen(true)}>💬 Chatbot</button>
            )}
            <OfflineIndicator />
            <ThemeToggle />
      ...
      {ASK_JESMANAGE_ROLES.includes(user?.role) && !askOpen && (
        <button
          type="button"
